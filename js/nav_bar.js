$(document).ready(function() {
    $.ajax({
        url: window.location.origin + "/SECURDEMukhlat/notifications",
        context: document.body
    });

    $.ajax({
        url: window.location.origin + "/SECURDEMukhlat/topic/refresh",
        context: document.body
    });
    
    $("#nav-logo").on("mouseover", function() {
        $(this).attr('src', window.location.origin + '/SECURDEMukhlat/images/logo/mukhlatlogo on the sideb.png');
    });
    ...
    $("#notif-btn").on("click", function() {
        ...
        $.ajax({
            url: window.location.origin + "/SECURDEMukhlat/notifications/read",
            success: function() {
                $("#notif-badge").remove();
    ...
    $(".invite-accept").on("click", function() {
        ...
        var id = inv_btn.val();
        $.ajax({
            url: window.location.origin + "/SECURDEMukhlat/invite/accept/invite/" + id,
            success: function(data) {
                ...
                var add_span = "<div class = 'invite-action text-success col-xs-12' style = 'padding: 15px; font-size: 12px;'>" +
                        ...
                        "<a class = 'btn btn-link btn-xs no-padding no-margin' style = 'padding-bottom: 3px;' href = '" + window.location.origin + "/SECURDEMukhlat/user/profile/" + data.user_id + "'>"
                        + "<strong>" + data.first_name + " " + data.last_name + "</strong></a>. " +
                        "Check out <a class = 'btn btn-link btn-xs no-padding no-margin' style = 'padding-bottom: 3px;' href = '" + window.location.origin + "/SECURDEMukhlat/topic/view/" + data.topic_id + "'>"
    ...
    $(".invite-decline").on("click", function() {
        ...
        var id = inv_btn.val();
        $.ajax({
            url: window.location.origin + "/SECURDEMukhlat/invite/decline/invite/" + id,
            success: function(data) {
                var add_span = "<div class = 'invite-action text-danger col-xs-12' style = 'padding: 15px; font-size: 12px;'>" +
                        ...
                        " <a class = 'btn btn-link btn-xs no-padding no-margin' style = 'padding-bottom: 3px;' href = '" + window.location.origin + "/SECURDEMukhlat/topic/view/" + data.topic_id + "'>"
                        + "<strong>" + data.topic_name + "</strong></a>!</div>";
                var list_item = inv_btn.closest('.notif-item');
                list_item.html(add_span);
    ...
    $(".request-accept").on("click", function() {
        ...
        $.ajax({
            url: window.location.origin + "/SECURDEMukhlat/invite/accept/request/" + id,
            success: function(data) {
                var add_span = "<div class = 'invite-action text-success col-xs-12' style = 'padding: 15px; font-size: 12px;'>" +
    ...
    $(".request-decline").on("click", function() {
        ...
        $.ajax({
            url: window.location.origin + "/SECURDEMukhlat/invite/decline/request/" + id,
            success: function(data) {
                var add_span = "<div class = 'invite-action text-danger col-xs-12' style = 'padding: 15px; font-size: 12px;'>" +
